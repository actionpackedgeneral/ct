...
class UserProfileDialog extends ComponentDialog {
  constructor() {
    ...
    const lgFile = Templates.parseFile(
      path.join(__dirname, "userProfileDialog.lg")
    );
            ...
            new IfCondition().configure({
              ...
              actions: [
                new NumberInput().configure({
                  ...
                  unrecognizedPrompt: new ActivityTemplate(
                    "${AskForAge.unRecognized()}"
                  ),
                }),
                new SendActivity("${UserAgeReadBack()}"),
              ],
              elseActions: [new SendActivity("${NoAge()}")],
            }),
            new IfCondition().configure({
              condition: new BoolExpression(
                'turn.activity.channelId == "msteams"'
              ),
              actions: [
